    ...
    def vis_omni(self, epoch, iter, num_iter, use_depth, use_color, c2w_or_camera_tensor, c,
        ...
        with torch.no_grad():
            if iter % self.inside_freq == 0:
                ...
                invdepth_np = colorMap('oliver', 1/depth_np, 1/gt_max_depth, 1/gt_min_depth)
                use_invdepth_np = colorMap('oliver', 1/use_depth_np, 1/gt_max_depth, 1/gt_min_depth)
                ...
                depth_np = colorMap('plasma', depth_np, gt_min_depth, gt_max_depth)
                use_depth_np = colorMap('plasma', use_depth_np, gt_min_depth, gt_max_depth)
                # gt_depth_np = colorMap('plasma', gt_depth_np, gt_min_depth, gt_max_depth)
        ...
        with torch.no_grad():
            if iter % self.inside_freq == 0:
                ...
                if summary_writer is not None:
                    prev_output = cv2.cvtColor(prev_output, cv2.COLOR_BGR2RGB)
                    prev_output = torch.tensor(prev_output, dtype=torch.float64).permute(2,0,1) / 255
                    summary_writer.add_image(f'{stage}', prev_output, iter)
